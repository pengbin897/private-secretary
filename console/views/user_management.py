...
from system.common.result import page_success, success, error
...
class UserDetailsView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        user_account = UserManageAccount.objects.create(
            user_id=user.pk,
            channel=UserChannel.TRANDITIONAL.value,
            balance=balence,
            level=level
        )
        return success()
    ...
    def delete(self, request: Request, user_id: int) -> Response:
        ''' 删除用户 '''
        UserManageAccount.objects.filter(user__id=user_id).delete()
